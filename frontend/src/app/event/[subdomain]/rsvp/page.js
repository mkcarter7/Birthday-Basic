    ...
    const loadExisting = async () => {
      ...
      try {
        ...
        if (Array.isArray(payload)) {
          record = payload.find((item) => String(item.party) === String(config.id)) || payload[0] || null;
        } else if (payload && typeof payload === 'object') {
          const maybeList = payload?.results || payload?.rsvps;
          if (Array.isArray(maybeList)) {
            record = maybeList.find((item) => String(item.party) === String(config.id)) || maybeList[0] || null;
          } else if (!payload.party || String(payload.party) === String(config.id)) {
            record = payload;
          }
        }
    ...
    <main className="page">
      ...
      {!user ? (
        <div className="card" style={{ display: 'grid', gap: 12 }}>
          <p>Sign in to RSVP for {config.name}!</p>
