...
function buildSmsHref(phone, body) {
  ...
  const isIOS = /iPad|iPhone|iPod/.test(typeof navigator !== 'undefined' ? navigator.userAgent : '')
    || (/Macintosh/.test(typeof navigator !== 'undefined' ? navigator.userAgent : '') && 'ontouchend' in document);
  if (isIOS) return phone ? `sms:${phone}&body=${encoded}` : `sms:&body=${encoded}`;
...
export default function SmsPage() {
  ...
  const partyUrl = useMemo(() => `${getFrontendUrl()}/`, []);
  const message = useMemo(
    () => `You're invited to ${config.name}! ${config.date} · ${config.time}. Details: ${partyUrl}`,
    [config.name, config.date, config.time, partyUrl],
  );

  const smsHref = useMemo(() => buildSmsHref(normalizePhone(phone), message), [phone, message]);

  const onSend = () => { setCopied(false); window.location.href = smsHref; };
    ...
    <main className="page">
      ...
      <div className="card" style={{ display: 'grid', gap: 12 }}>
        ...
        <p className="muted" style={{ margin: 0 }}>
          Tip: On desktop, the SMS link may open your default messaging app. Best results on a phone.
        </p>
      </div>
    </main>
  );
