...
export default function GameDetailPage({ params }) {
  ...
  const getGameTitle = () => {
    switch (id) {
      case 'trivia': return 'Party Trivia';
  ...
  const getGameSubtitle = () => {
    switch (id) {
      case 'trivia': return 'Test your knowledge and earn points!';
      default: return 'Demo game screen';
    }
  };
  ...
  const renderGameContent = () => {
        ...
        <div className="card" style={{ display: 'grid', gap: 16, textAlign: 'center' }}>
          ...
          {results && (
            <>
              ...
              {fullResponse?.question_results?.length > 0 && (
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 14, color: '#6b7280', marginTop: 8 }}>
                              <div><strong>Your answer:</strong> Option {String.fromCharCode(65 + qResult.your_answer)} ({qResult.your_answer})</div>
                              {!qResult.is_correct && (
          ...
          <p className="muted" style={{ marginTop: 16 }}>Your score has been saved to the leaderboard.</p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 8 }}>
            ...
            <button
              type="button"
              onClick={() => { setGameComplete(false); setFinalScore(0); setResults(null); window.location.reload(); }}
    ...
    if (id === 'trivia') {
      return <TriviaGame onComplete={handleGameComplete} partyId={config.id} />;
    }
