...
export default function GamesPage() {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      ...
      try {
        if (user) {
          const score = await getUserScore(user, config.id);
          setUserScore(score);
        }

        const lb = await getLeaderboard(user, config.id);
        setLeaderboard(lb);
      } catch (error) {
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {demoGames.map((game) => (
            <Link
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 20, textDecoration: 'none', border: 'none' }}
            >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div
                      style={{
                        ...
                        height: 32,
                        borderRadius: '50%',
                        background: index === 0 ? '#fbbf24' : index === 1 ? '#94a3b8' : index === 2 ? '#f97316' : 'rgba(139, 92, 246, 0.2)',
                        display: 'flex',
                        alignItems: 'center',
