...
from birthdayapi.views.site_config import SiteConfigViewSet
from birthdayapi.views.subscription import SubscriptionViewSet
from birthdayapi.views.checkout import CreateCheckoutSessionView, StripeWebhookView
...
router.register(r'api/site-config', SiteConfigViewSet, basename='siteconfig')
...
urlpatterns = [
    ...
    # Stripe
    path('api/checkout/create-session/', CreateCheckoutSessionView.as_view(), name='checkout_create_session'),
...
# Serve media files
# If using S3, media files are served directly from S3, so no Django serving needed
USE_S3 = os.getenv('USE_S3', 'False').lower() == 'true'
AWS_ACCESS_KEY_ID = os.getenv('AWS_ACCESS_KEY_ID')
AWS_STORAGE_BUCKET_NAME = os.getenv('AWS_STORAGE_BUCKET_NAME')

if not (USE_S3 and AWS_ACCESS_KEY_ID and AWS_STORAGE_BUCKET_NAME):
    # Only serve media files locally if not using S3
    if settings.DEBUG:
        urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)
        urlpatterns += static(settings.STATIC_URL, document_root=settings.STATIC_ROOT)
    else:
        # In production with local storage, serve media files through Django
